...
# ═══ MODIF 5.13 — require_module sur le router ═══
router = APIRouter(
    prefix="/biens",
    tags=["Biens"],
...
# ═══ AJOUT 5.13 — Helpers d'isolation multi-tenant ═══
def _is_platform_admin(user: Utilisateur) -> bool:
    return bool(getattr(user, "is_platform_admin", False))


def _check_acces_bien(bien, current_user: Utilisateur) -> None:
    """Vérifie l'accès à un Bien. Lève 403 si interdit."""
    ...
    if bien_org is None:
        raise HTTPException(
            ...
            detail=(
                f"Accès refusé : bien #{getattr(bien, 'id_bien', '?')} sans organisation "
    ...
    if bien_org != current_user.organisation_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"Accès refusé : bien #{getattr(bien, 'id_bien', '?')} appartient à une autre organisation.",
        )


def _filtre_biens_ong(biens: list, current_user: Utilisateur) -> list:
    """Filtre une liste de biens selon l'organisation (post-filtre temporaire)."""
    if _is_platform_admin(current_user):
        return biens
    org_id = current_user.organisation_id
    return [b for b in biens if getattr(b, "organisation_id", None) == org_id]
# ═══ FIN AJOUT 5.13 ═══
...
def get_biens(
    ...
    try:
        ...
        # ═══ MODIF 5.13 — Post-filtre isolation ONG ═══
        # ⚠️ Post-filtre temporaire : le service n'accepte pas organisation_id.
        # À terme, il faudra modifier BienService.get_all_biens/get_biens_count
        # pour filtrer au niveau SQL.
        biens_filtres = _filtre_biens_ong(biens, current_user)
        if not _is_platform_admin(current_user):
            # On renvoie un total re-calculé approximatif sur la page courante
            total_count = len(biens_filtres)
        # ═══ FIN MODIF 5.13 ═══
        ...
        return BienListResponse(
            ...
            page=(skip // limit) + 1,
            page_size=limit,
            biens=[_to_bien_response(b, current_user) for b in biens_filtres],
        )
    except HTTPException:
        raise
    except Exception as e:
...
async def get_bien(
    ...
    try:
        ...
        # ═══ MODIF 5.13 — Vérification d'accès ONG ═══
        _check_acces_bien(bien, current_user)
        # ═══ FIN MODIF 5.13 ═══
        ...
        if isinstance(e, ValidationError):
            raise HTTPException(status_code=500, detail=f"Erreur Pydantic: {e.errors()}")
        if isinstance(e, ValueError):
...
async def update_bien(
    ...
    if not can_update_bien(current_user):
        _deny(current_user, "update_bien", "Permissions insuffisantes pour modifier un bien", request, bien_id)
    ...
    # ═══ MODIF 5.13 — Vérification d'accès ONG ═══
    _check_acces_bien(old_bien, current_user)
    # ═══ FIN MODIF 5.13 ═══

    filtered_data = filter_bien_update(current_user, bien_data)
    if not filtered_data.model_dump(exclude_unset=True):
        _deny(current_user, "update_bien_empty", "Aucun champ modifiable autorisé pour votre rôle", request, bien_id)
...
async def delete_bien(
    ...
    if not can_delete_bien(current_user):
        _deny(current_user, "delete_bien", "Permissions insuffisantes pour supprimer un bien", request, bien_id)
...
async def change_bien_etat(
    ...
    if not can_update_bien(current_user):
        _deny(current_user, "change_etat", "Permissions insuffisantes pour modifier l'état d'un bien", request, bien_id)
...
def get_biens_eligibles_cession(
    ...
    # ═══ MODIF 5.13 — Filtre ONG sur la requête ═══
    query = db.query(Bien).options(
        joinedload(Bien.localisation_ref),
        selectinload(Bien.amortissements),
        selectinload(Bien.pannes)
    )
    if not _is_platform_admin(current_user):
        query = query.filter(Bien.organisation_id == current_user.organisation_id)
    biens = query.all()
    # ═══ FIN MODIF 5.13 ═══
...
async def demander_cession(
    ...
    try:
        with db.begin():
            ...
            if data.actif_remplacement_id:
                ...
                # ═══ MODIF 5.13 — Vérification du bien de remplacement ═══
                _check_acces_bien(remplacement, current_user)
                # ═══ FIN MODIF 5.13 ═══
            ...
            # ═══ MODIF 5.13 — organisation_id sur validations ═══
            org_id_bien = getattr(bien, "organisation_id", None)
            # ═══ FIN MODIF 5.13 ═══
            ...
            validation_comptable = Validation(
                ...
                organisation_id=org_id_bien,  # ═══ AJOUT 5.13 ═══
            )
    ...
    # ═══ MODIF 5.13 — Notifications filtrées par ONG ═══
    try:
        notification_service = NotificationService(db)
        comptables = validation_service._get_utilisateurs_par_roles(
            "COMPTABLE", organisation_id=org_id_bien,
        )
...
async def get_score_fiabilite(
    ...
    return {
        ...
        "historique": [{"date": h.date_evenement, "score": h.montant} for h in historique_scores],
        "maintenances_auto": [
            {"id": m.id_maintenance, "date_planifiee": m.date_planifiee,
             "statut": m.statut.value if m.statut else None, "score_depart": m.score_fiabilite_depart}
            for m in maintenances_auto
        ],
        "ordres_remplacement": [
            {"id": o.id, "motif": o.motif, "statut": o.statut.value if o.statut else None,
             "date_creation": o.date_creation, "priorite": o.priorite.value if o.priorite else None}
            for o in ordres
        ]
    }
...
async def get_biens_critiques(
    ...
    # ═══ MODIF 5.13 — Filtre ONG ═══
    query = db.query(Bien).filter(
        Bien.est_critique == True,
        Bien.statut_comptable == 'ACTIF'
    )
    if not _is_platform_admin(current_user):
        query = query.filter(Bien.organisation_id == current_user.organisation_id)
    biens = query.order_by(Bien.score_fiabilite.asc()).all()
    # ═══ FIN MODIF 5.13 ═══
...
async def get_maintenances_auto_recents(
    ...
    # ═══ MODIF 5.13 — Filtre ONG via join Bien ═══
    ...
    maintenances = query.order_by(Maintenance.date_creation.desc()).limit(limit).all()
    # ═══ FIN MODIF 5.13 ═══
